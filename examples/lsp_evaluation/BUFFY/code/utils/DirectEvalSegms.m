...
for p = 1:Np
  ...
  % classify
  ndxy = abs(ndxy);
  if isfield(EvalChoice, 'both') && EvalChoice.both == 0
      avg_ndxy = [mean(ndxy([1 3])) mean(ndxy([2 4]))];
      R(p) = (avg_ndxy(1) < pars.max_parall_dist) && (avg_ndxy(2) < pars.max_perp_dist);
  else
      R(p) = (ndxy(1) < pars.max_parall_dist) && (ndxy(3) < pars.max_parall_dist) && (ndxy(2) < pars.max_perp_dist) && (ndxy(4) < pars.max_perp_dist);
  end
  %if p == 4
  %    keyboard;
  %end
