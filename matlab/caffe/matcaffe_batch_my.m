function [scores, list_im, real_label, accuracy] = matcaffe_batch(list_im, use_gpu, model_n, deploy_name, model_name)
...
if ischar(list_im)
    ...
    filename = list_im;
    [list_im, real_label] = read_cell_my(filename);
end
...
matcaffe_init(use_gpu, deploy_name, model_name);

% d = load('trafficSign_mean');
% IMAGE_MEAN = d.trafficSign_mean;
IMAGE_MEAN_temp = caffe('read_mean', '/home/cv/image-net/caffe/data/trafficSignRGB/trafficSign_mean.binaryproto');
IMAGE_MEAN = permute(IMAGE_MEAN_temp, [2 1 3]);
% prepare input
...
accuracy = sum(right_label)/length(right_label);

if exist('filename', 'var')
    save([filename, num2str(model_n), '.probs.mat'],'list_im','scores','real_label','predict_label','accuracy','-v7.3');
end
