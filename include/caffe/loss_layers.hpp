...
/**
 * @brief Computes the Regression accuracy
 */
template <typename Dtype>
class AccuracyRegressionLayer : public Layer<Dtype> {
 public:
  /**
   * @param param provides AccuracyRegressionParameter accuracy_regression_param,
   *     with AccuracyLayer options:
   *   - top_k (\b optional, default 1).
   *     Sets the maximum rank @f$ k @f$ at which a prediction is considered
   *     correct.  For example, if @f$ k = 5 @f$, a prediction is counted
   *     correct if the correct label is among the top 5 predicted labels.
   */
  explicit AccuracyRegressionLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
 ...
 */
template <typename Dtype>
class CustomEuLossLayer : public LossLayer<Dtype> {
 public:
  explicit CustomEuLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_() {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top);

  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_CUSTOM_EU_LOSS;
  }
 ...
 protected:
  ...
  // Custom parameter
  bool 			is_selected_;
  vector<int> 	selected_point_;
  Blob<Dtype> 	custom_label_;
};
